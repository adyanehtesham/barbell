...
const WeightPlan = (availableWeights, sequence) => {

    let source = availableWeights
    let seq = sequence
    ...
    // Get best strategy
    allPaths.sort((a, b) => a.reduce((c, d) => c + d[2], 0) - b.reduce((c, d) => c + d[2], 0)).slice(0, 10) 

    // console.log("most optimal path to minimize number of plates moved")
    const optimumPath = allPaths[0]
    // console.log(allPaths[0])

    const steps = optimumPath.map(step => step[1])
